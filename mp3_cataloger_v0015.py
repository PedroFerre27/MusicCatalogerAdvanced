...
class MP3CatalogerAdvanced:
    ...
    def search_lastfm(self, artist: str, title: str) -> Optional[Dict]:
        """Cerca metadati su Last.fm con attributi estesi"""
        ...
        try:
            ...
            # Prima chiamata: ottieni info base del track
            track_params = {
            ...
            # NUOVO: Estrai album info se disponibile
            if track.get('album'):
                album_info = track['album']
                metadata['album'] = album_info.get('title')
                
                # NUOVO: Prova a ottenere l'anno dal release date dell'album
                if album_info.get('mbid'):  # Se abbiamo MusicBrainz ID
                    album_mbid = album_info['mbid']
                    try:
                        # Seconda chiamata: ottieni info album
                        album_params = {
                        ...
                        self.logger.debug(f"Errore recupero info album Last.fm: {e}")
                
                # Fallback: cerca anno nell'artist info se non trovato nell'album
                if not metadata.get('year'):
                    try:
                        # Terza chiamata: ottieni info artista per possibili date
                        artist_params = {
                        ...
                        if artist_response.status_code == 200:
                            ...
                            if 'artist' in artist_data and artist_data['artist'].get('bio'):
                                bio = artist_data['artist']['bio']
                                if bio.get('published'):
                                    # Cerca anno nella biografia
                                    published = bio['published']
                                    year_match = re.search(r'\b(19|20)\d{2}\b', published)
                                    if year_match:
                                        # Usa come fallback, ma con priorità bassa
                                        metadata['year_estimated'] = year_match.group(0)
                    
                    except Exception as e:
                        self.logger.debug(f"Errore recupero info artista Last.fm: {e}")
            
            # NUOVO: Estrai durata se disponibile
            if track.get('duration'):
                try:
                    # Last.fm restituisce durata in millisecondi
                    duration_ms = int(track['duration'])
                    if duration_ms > 0:
                        metadata['duration'] = duration_ms / 1000.0  # Converti in secondi
                except (ValueError, TypeError):
                    pass
            
            # Estrai generi dai tag (esistente, ma migliorato)
            if track.get('toptags', {}).get('tag'):
                tags = track['toptags']['tag']
                if isinstance(tags, list) and tags:
                    # Prendi il primo tag con peso maggiore
                    primary_tag = tags[0]
                    if isinstance(primary_tag, dict):
                        metadata['genre'] = primary_tag.get('name')
                    else:
                        metadata['genre'] = str(primary_tag)
                elif isinstance(tags, dict):
                    metadata['genre'] = tags.get('name')
            
            # NUOVO: Stima BPM da tag se disponibile
            if track.get('toptags', {}).get('tag'):
                tags = track['toptags']['tag']
                if isinstance(tags, list):
                    for tag in tags:
                        tag_name = tag.get('name', '').lower() if isinstance(tag, dict) else str(tag).lower()
                        # Cerca tag che potrebbero indicare BPM/tempo
                        bpm_indicators = {
                            'slow': '70', 'ballad': '75', 'medium': '100', 
                            'upbeat': '120', 'fast': '140', 'energetic': '130',
                            'dance': '125', 'club': '128', 'house': '125',
                            'techno': '130', 'trance': '135'
                        }
                        if tag_name in bpm_indicators:
                            metadata['bpm_estimated'] = bpm_indicators[tag_name]
                            break
            
            # NUOVO: Popolarità/playcount come metrica aggiuntiva
            if track.get('playcount'):
                try:
                    playcount = int(track['playcount'])
                    metadata['playcount'] = playcount
                    
                    # Categorizza la popolarità
            ...
            # NUOVO: URL Last.fm per riferimento
            if track.get('url'):
    ...
    # NUOVO: Metodo per unire metadati con priorità intelligente
