# Note V0016:
#MusicBrainz - Estrazione Generi Potenziata:
#Tag del Recording: Estrae generi direttamente dal brano
#Tag dell'Artista: Estrae generi dall'artista principale
#Filtro Intelligente: Distingue tra generi musicali reali e tag generici
#Priorità per Generi Latini: Da priorità a salsa, bachata, merengue, etc.
#
#Last.fm - Estrazione Generi Multipla:
#Tag del Track: Generi specifici del brano
#Tag dell'Artista: Generi dall'artista (fallback)
#Tag dell'Album: Generi dall'album (se disponibile)
#Chiamate API Multiple: Fino a 3 chiamate per completezza massima
#
#Funzioni di Supporto Aggiunte:
#_is_music_genre_tag(): Filtra tag che sono realmente generi musicali
#_select_primary_genre(): Seleziona il genere più rilevante da una lista
#
#Priorità Generi Intelligente:
#La funzione dà priorità a:
#Generi Latini (per il tuo caso specifico)
#Generi Specifici (es. "progressive rock" vs "rock")
#Generi Comuni (rock, pop, jazz)
#Generi Generici (alternative, indie)
#
#Debug e Cache:
#Salva tutti i generi trovati in all_genres per debug
#Cache separata per evitare chiamate ripetute
...
class MP3CatalogerAdvanced:
    def __init__(self, base_path: str, log_level=logging.DEBUG, dry_run=False, use_external_db=True):
        ...
        if musicbrainzngs and self.use_external_db:
            musicbrainzngs.set_useragent("MusicCatalogerAdvanced", "v0016", "captainjoker27@gmail.com")
        ...
        try:
            ...
            try:
                ...
                if genres:
                    ...
                    self.logger.info(f"MusicBrainz: generi trovati per {artist} - {title}: {unique_genres}")
                    
                    primary_genre = self._select_primary_genre(unique_genres)
                    if primary_genre:
                        metadata['genre'] = primary_genre
                        self.logger.info(f"MusicBrainz: genere primario selezionato: {primary_genre}")
                    
                    metadata['all_genres'] = unique_genres[:5]
                else:
                    self.logger.debug(f"MusicBrainz: nessun genere trovato per {artist} - {title}")
                
                self.logger.info(f"MusicBrainz: metadati completi per {artist} - {title}: {list(metadata.keys())}")
    ...
    def estimate_bpm_from_audio(self, file_path: Path) -> Optional[int]:
        """Stima il BPM dall'analisi audio usando librosa"""
    ...
    def process_mp3_file(self, file_path: Path):
        """CORRETTO: Processa un singolo file MP3 con controllo duplicati"""
        ...
        try:
            ...
            if self.use_external_db and search_artist and search_title:
                ...
                try:
                    ...
                    if external_metadata:
                        self.logger.info(f"MusicBrainz: successo per {file_path.name}")
                    else:
                ...
                if not external_metadata:
                    ...
                    try:
                        ...
                        if external_metadata:
                            self.logger.info(f"Last.fm: successo per {file_path.name}")
                        else:
            ...
            # 6. Normalize genre with enhanced logging
            self.logger.debug("Fase 5: Normalizzazione genere")
            raw_genre = final_metadata.get('genre')
            ...
            if not genre or genre == "Other":
                ...
                if found_indicator:
                    genre = 'Latin'
                    self.logger.info(f"Genere Latino inferito da indicatore: '{found_indicator}' in {file_path.name}")
                else:
                    genre = 'Unknown'
                    self.logger.info(f"Genere sconosciuto per {file_path.name} - assegnato 'Unknown'")
            
            # 7. Move to genre folder with better error handling
    ...
    def move_to_genre_folder(self, file_path: Path, genre: str, metadata: Dict) -> bool:
        ...
        try:
            # Verifica se il genere stesso è un sottogenere Latino
            final_genre_folder = Path(self.clean_filename(genre))
            
            # Controlla se il genere è nella lista dei sottogeneri di 'Latin'
            if genre.lower() in [s.lower() for s in self.subgenre_mapping.get('Latin', [])]:
                # Crea la cartella del sottogenere sotto 'Latin'
                final_genre_folder = Path('Latin') / self.clean_filename(genre.capitalize())
            else:
            ...
            if moved_name != original_name:
                self.logger.info(f"Spostato {original_name} -> {final_genre_folder}/{moved_name} (rinominato)")
            else:
                self.logger.info(f"Spostato {original_name} -> {final_genre_folder}/")
