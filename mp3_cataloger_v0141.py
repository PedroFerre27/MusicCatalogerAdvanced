...
class MP3CatalogerAdvanced:
    def __init__(self, base_path: str, log_level=logging.INFO, dry_run=False, use_external_db=True):
        ...
        if musicbrainzngs and self.use_external_db:
            musicbrainzngs.set_useragent("MusicCataloger", "0.1.4.1", "captainjoker27@gmail.com")
    ...
    def search_musicbrainz(self, artist: str, title: str, album: str = None) -> Optional[Dict]:
        ...
        try:
            ...
            try:
                ...
                # Stima BPM se disponibile librosa
    ...
    def process_mp3_file(self, file_path):
        ...
        try:
            # 1. Estrai metadati esistenti con una migliore gestione degli errori
            existing_metadata = {}
            ...
            try:
                ...
                self.logger.debug(f"Fallito eyed3 per metadati {file_path.name}: {e}")
            
            # Prova mutagen come fallback
            if not existing_metadata:
                try:
                    existing_metadata = self.extract_metadata_mutagen(file_path) or {}
                except Exception as e:
                    self.logger.debug(f"Fallito Mutagen per metadati {file_path.name}: {e}")
            
            # 2. Deduci metadati dal filename
            ...
            if self.use_external_db and search_artist and search_title:
                ...
                if not external_metadata:
                    try:
                        ...
                        self.logger.debug(f"Ricerca Last.fm fallita: {e}")
            ...
            if final_metadata and not self.dry_run:
                try:
                    ...
                    self.logger.warning(f"Fallito aggiornamento metadata per {file_path.name}: {e}")
            ...
            # Se non trovati generi, deduci dal titolo o artista
            if not genre or genre == "Other":
                # Controlla se è una musica latina
                ...
                if any(indicator in artist_name for indicator in latin_indicators):
                    genre = 'Latin'
                    self.logger.info(f"Dedotto genere Latino per {file_path.name}")
                else:
                    # Default "Unknown" Invece di fallire
                    genre = 'Unknown'
                    self.logger.info(f"Uso 'Unknown' genere per {file_path.name}")
            
            # 9. Crea cartella genere e muovi il file
            ...
            self.logger.error(f"Errore inaspettato durante il processo {file_path.name}: {e}")
            self.uncatalogued_files.append({
                'file': file_path.name,
    ...
    def move_to_genre_folder(self, file_path: Path, genre: str, metadata: Dict) -> bool:
        ...
        try:
            ...
            if self.dry_run:
                self.logger.info(f"[SIMULATIONE] Sposterei {file_path.name} -> {genre}/")
                return True
            
            # Crea cartella genere
            genre_folder.mkdir(exist_ok=True)
            
            # Gestisci conflitti nomenclatura
            ...
            self.logger.error(f"Errore spostamento {file_path.name} verso {genre}: {e}")
            return False
